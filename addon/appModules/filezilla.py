...
class AppModule(appModuleHandler.AppModule):
	@scriptHandler.script(
		gesture="kb:control+f6",
		description=_(
			"FileZilla: Switch between local and remote file lists; when not in a list, navigate to the remote file list."
		),
		category="FileZilla",
	)
	def script_switchList(self, gesture):
		try:
			...
			ui.message(
				_(
					"Unable to access the list because the FileZilla version is not detected"
				)
			)

	@scriptHandler.script(
		gesture="kb:control+shift+h",
		description=_("FileZilla: go to the connections history button"),
		category="FileZilla",
	)
